...
BATT_NOM_KWH=84.48; GRID_CI=0.384; YEAR=2025
...
hdr(ws3,1,1,"Q8 – Final Battery Bank Design | 4S×5P | 20 × Victron LFP 12.8V/330Ah",H,sz=12,span=4,rh=24)
...
bank_rows=[
    ("Battery type",                "Victron LFP Smart",               "12.8V / 330Ah",  "Integrated BMS, 2500 cycles @ 80% DoD"),
    ("Energy per unit",             "=12.8 × 330 / 1000",              "4.224 kWh",      ""),
    ("DC bus voltage (inverter)",   "XW Pro 6848 requirement",         "48V nominal",    ""),
    ("Series units per string",     "=4 × 12.8V",                      "4S → 51.2V",     ""),
    ("String capacity (Ah)",        "Series: same Ah",                  "330 Ah",         "Series connection keeps Ah unchanged"),
    ("String energy",               "=51.2 × 330 / 1000",              "16.896 kWh",     ""),
    ("Parallel strings",            "ceil(65.65 / 16.896)",             "4P would give 67.58kWh; 5P selected for margin",""),
    ("TOTAL BATTERIES",             "=4S × 5P",                        "20 batteries",   "Within 20-unit BMS limit ✓"),
    ("Bank voltage",                "=4 × 12.8",                       "51.2 V",         ""),
    ("Bank total Ah",               "=5 × 330",                        "1,650 Ah",       ""),
    ("NOMINAL CAPACITY",            "=20 × 4.224",                     "84.48 kWh",      ""),
]
...
tbox(ws3,28,1,4,
    ...
    f"usable = 80% × 84.48 = 67.58 kWh → margin = {84.48*0.80-DC_per_event:.2f} kWh\n"
    "  • Interpretation B (DoD from full charge 100%): floor = 100% − 80% = 20% SoC → "
    f"usable = 75% × 84.48 = 63.36 kWh → margin = {84.48*0.75-DC_per_event:.2f} kWh\n"
...
hdr(ws5,1,1,"Q9 – Charging & Discharging Currents | 4S×5P | 51.2V / 1,650Ah",H,sz=12,span=4,rh=24)
...
disch=[
    ...
    ("Bank capacity","5P × 330Ah","1,650 Ah",""),
    ("Discharge C-rate",f"= {disc_A:.2f} ÷ 1650",f"{disc_A/1650:.4f} C","Very gentle"),
    ("Bank rated max","5P × 400A/unit","2,000 A",""),
    ("Safety margin",f"= 2000 ÷ {disc_A:.2f}",f"{2000/disc_A:.1f}×","Well within limits"),
]
...
charg=[
    ...
    (f"Charge C-rate at {pk_A:.1f}A",  f"= {pk_A:.2f} ÷ 1650",         f"{pk_A/1650:.4f} C","Extremely safe"),
    ("LFP max rated (≤1C)",             "1 × 1650Ah",                    "1,650 A max",    f"We use {pk_A/1650*100:.1f}% of max"),
]
...
tbox(ws5,25,1,4,
    "DISCHARGE: Max 128.2A is 0.077C on a 1,650Ah bank — 15.6× below the rated 2,000A. "
    ...
    "but the actual PV production never drives charging above 182.4A at the 51.2V bus. "
    "The PV array — not the controller rating — is the governing limit. "
    "At 182.4A the C-rate is 0.110C, well within LFP safe-charge limits, preserving the "
